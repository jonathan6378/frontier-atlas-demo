...
"""
    ...
    def get_rate_limit_cooldown(
        ...
        """
        Extract the provider's requested retry/cooldown time
        from a rate-limit error.

        Examples:
            "Please try again in 8m58.704s"
            "Please retry in 10.274039105s"
            "retry in 44 seconds"
        """
        ...
        #
        #   retry in 8m58.704s
        #   try again in 8m58.704s
        #   retrying in 8m58.704s
        # --------------------------------------------------------

        match = re.search(
            r"(?:retry(?:ing)?|try\s+again)"
            r"\s+(?:in\s+)?"
            r"(?:(\d+)\s*m)?"
            r"\s*(\d+(?:\.\d+)?)\s*s",
            text,
        ...
        if match:
            ...
            # Add a small safety buffer so we don't retry
            # exactly when the provider's timer expires.
            cooldown = int(
                total_seconds + 3
            )

            logger.info(
                "Parsed provider retry time: %ss",
                cooldown,
            )
        ...
        logger.warning(
            "Could not determine provider retry time. "
            "Using 60s fallback."
        )
